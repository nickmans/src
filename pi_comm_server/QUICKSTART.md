...
3. Send `traj2 2` from STM32 to keep localization active but stay manual on STM32 (trajectory output disabled).
4. Send `traj 3` from STM32 to run autonomous waypoint following with a blank global map and local-only obstacle avoidance.
5. Send `traj 0` from STM32 to return to standby/manual behavior.
